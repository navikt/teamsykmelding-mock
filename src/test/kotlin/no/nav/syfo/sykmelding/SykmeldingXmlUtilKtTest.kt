    ...
    FunSpec({
        context("LagHelseopplysninger") {
            ...
            test("Helseopplysninger opprettes korrekt for maksimal request") {
                ...
                helseopplysninger.medisinskVurdering.hovedDiagnose.diagnosekode.dn shouldBeEqualTo
                    "Ganglion"
                helseopplysninger.medisinskVurdering.annenFraversArsak.arsakskode[0]
                    .dn shouldBeEqualTo
                ...
                helseopplysninger.aktivitet.periode[1].periodeTOMDato shouldBeEqualTo
                    LocalDate.now().plusWeeks(1)
                helseopplysninger.aktivitet.periode[1]
                    .gradertSykmelding
                    .sykmeldingsgrad shouldBeEqualTo 60
                helseopplysninger.aktivitet.periode[1]
                    .gradertSykmelding
                    .isReisetilskudd shouldBeEqualTo true
                helseopplysninger.kontaktMedPasient.behandletDato shouldBeEqualTo
                    LocalDate.now().atStartOfDay()
                helseopplysninger.kontaktMedPasient.kontaktDato shouldBeEqualTo
                    LocalDate.now().minusDays(2)
                helseopplysninger.kontaktMedPasient.begrunnIkkeKontakt shouldBeEqualTo
                    "Hadde ikke tid"
                helseopplysninger.behandler.id[0].id shouldBeEqualTo "10987654321"
            ...
            test("Helseopplysninger opprettes korrekt for maksimal request og regelsettversjon 3") {
                ...
                helseopplysninger.medisinskVurdering.biDiagnoser.diagnosekode[0].v shouldBeEqualTo
                    "M674"
                helseopplysninger.medisinskVurdering.biDiagnoser.diagnosekode[1].v shouldBeEqualTo
                    "Z09"
                helseopplysninger.arbeidsgiver.navnArbeidsgiver shouldBeEqualTo "NAV"
            }
            test(
                "Helseopplysninger kan opprettes med tullekode som diagnosekode (teste avvist sykmelding)"
            ) {
                val sykmeldingRequest =
                    SykmeldingRequest(
                        fnr = "12345678910",
