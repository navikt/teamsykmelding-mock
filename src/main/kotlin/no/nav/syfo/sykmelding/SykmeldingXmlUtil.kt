...
fun lagHelseopplysninger(
    ...
    return HelseOpplysningerArbeidsuforhet().apply {
        ...
        tiltak =
            if (sykmeldingRequest.regelsettVersjon == "3") {
                ...
                HelseOpplysningerArbeidsuforhet.Tiltak().apply {
                    ...
                    tiltakNAV =
                        "Pasienten har plager som er kommet tilbake etter operasjon. Det er nylig tatt MR bildet som " +
        ...
        utdypendeOpplysninger =
            if (sykmeldingRequest.utdypendeOpplysninger == UtdypendeOpplysninger.INGEN) {
                null
            } else if (sykmeldingRequest.regelsettVersjon == "3") {
                HelseOpplysningerArbeidsuforhet.UtdypendeOpplysninger().apply {
                    spmGruppe.addAll(
                        tilSpmGruppeRegelsett3(sykmeldingRequest.utdypendeOpplysninger)
                    )
                }
            } else {
                HelseOpplysningerArbeidsuforhet.UtdypendeOpplysninger().apply {
...
private fun medisinskVurdering(
    sykmeldingRequest: SykmeldingRequest
    ...
    val medisinskVurdering =
        HelseOpplysningerArbeidsuforhet.MedisinskVurdering().apply {
            ...
            if (!sykmeldingRequest.bidiagnoser.isNullOrEmpty()) {
                biDiagnoser =
                    HelseOpplysningerArbeidsuforhet.MedisinskVurdering.BiDiagnoser().apply {
                        diagnosekode.addAll(
                            sykmeldingRequest.bidiagnoser.map { tilDiagnosekode(it) }
                        )
                    }
            }
            ...
            annenFraversArsak =
                sykmeldingRequest.annenFraverGrunn?.let {
                    ArsakType().apply {
                        arsakskode.add(
                            CS().apply {
                                v = it.codeValue
...
private fun tilPeriode(
    ...
    val xmlperiode =
        HelseOpplysningerArbeidsuforhet.Aktivitet.Periode().apply {
            ...
            aktivitetIkkeMulig =
                if (periode.type == SykmeldingType.HUNDREPROSENT) {
                    hundreprosent()
                } else null
            avventendeSykmelding =
                if (periode.type == SykmeldingType.AVVENTENDE) {
                    avventende()
                } else null
            gradertSykmelding =
                when (periode.type) {
...
private fun behandlingsdager(
    antallBehandlingsdager: Int
): HelseOpplysningerArbeidsuforhet.Aktivitet.Periode.Behandlingsdager {
