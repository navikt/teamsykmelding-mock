        ...
        return mutex.withLock {
            (tokenMap[resource]?.takeUnless { it.expiresOn.isBefore(omToMinutter) }
                ?: run {
                    ...
                    val tokenExchangeResponse = httpClient
                    ...
                    if (tokenExchangeResponse.status != HttpStatusCode.OK) {
                        throw IllegalStateException("Feil ved henting av token fra Azure AD, ${tokenExchangeResponse.status}, body: ${tokenExchangeResponse.bodyAsText()}")
                    }
